  ...
  return (
    <div className="space-y-2 p-2 max-w-screen-2xl mx-auto">
      {members.map((member) => (
        <div
          key={member.id}
          className="flex flex-col gap-2 md:flex-row items-center justify-between bg-gray-50 p-1 shadow-sm rounded-sm cursor-pointer"
          onClick={() => handleConversationClick(member.user.id)}
        >
