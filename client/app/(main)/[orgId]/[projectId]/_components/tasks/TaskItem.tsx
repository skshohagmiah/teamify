...
const TaskItem = ({ task, members,index,key }: TaskItemProps) => {
  ...
  return (
    <Draggable draggableId={task.id} index={index}>
      {(provided) => (
             <Link href={`/${orgId}/${projectId}/tasks/${task.id}`} {...provided.dragHandleProps} {...provided.draggableProps} ref={provided.innerRef} className="p-4 w-full max-w-[420px] flex flex-col gap-2 items-center justify-center rounded-md bg-gray-50 shadow-md  relative">
             ...
             <p>Assigned To {task.assignee.map((ass) => showMember(ass))}</p>
           </Link>
      )}
    </Draggable>
  );
